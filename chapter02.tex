...
%%=========================================
\chapter[Modelling and simulations]{Modelling and simulations}
\section{Section in python}
Here is a chapter

\section{Plant model}
The information in this master project is based on the master thesis of Gjermund Bae and Jakob Tiller.

Irradiance
\begin{equation}
U_{par} = c_p \cdot \text{PPFD} \, [\text{W m}^{-2}]
\end{equation}


Dry weight of crop per square meter of growth surface area.
\begin{equation}
X_{dw} = X_{nsdw} + X_{sdw} \, [\text{g m}^{-2}]
\end{equation}

\begin{subequations}
    \begin{equation}
        \frac{dX_{sdw}}{dt} = r_{gr} X_{sdw} \, [\text{g m}^{-2} \text{s}^{-1}]
    \end{equation}
    \begin{equation}
        \frac{dX_{nsdw}}{dt} = c_{\alpha} f_{phot} - r_{gr} X_{sdw} - f_{resp} - \frac{1 - c_{\beta}}{c_{\beta}} r_{gr} X_{sdw} \, [g \, m^{-2} \, s^{-1}]
    \end{equation}
\end{subequations}
